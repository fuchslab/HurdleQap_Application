...
W <- convert2df(file = "Data/databases/12_10_2020/BI_wosCoreCollection1.txt", dbsource="isi",format="plaintext")
for(i in 2:12){ 
  filename <- paste0("Data/databases/12_10_2020/BI_wosCoreCollection", i, ".txt")
  X <- convert2df(filename, dbsource="isi",format="plaintext")
  # merge data sets
  W <- bind_rows(W, X)
}
...
# scopus
O <- convert2df(file = "Data/databases/12_10_2020/BI_scopus2015.bib", dbsource = "scopus", format = "bibtex")
for(i in 2016:2019){
  filename <- paste0("Data/databases/12_10_2020/BI_scopus", i, ".bib")
...
# 2015
myquery1 <- "(Universitat Bielefeld[Affiliation] OR University Bielefeld[Affiliation] OR 
Bielefeld University[Affiliation] OR Universitaet Bielefeld[Affiliation] OR 
University of Bielefeld[Affiliation] OR Universität Bielefeld[Affiliation] AND (2015/01/01[PDat] : 2015/12/31[PDat]))" 


# 2016
myquery2 <- "(Universitat Bielefeld[Affiliation] OR University Bielefeld[Affiliation] OR 
Bielefeld University[Affiliation] OR Universitaet Bielefeld[Affiliation] OR 
University of Bielefeld[Affiliation] OR Universität Bielefeld[Affiliation] AND (2016/01/01[PDat] : 2016/12/31[PDat]))" 


# 2017
myquery3 <- "(Universitat Bielefeld[Affiliation] OR University Bielefeld[Affiliation] OR 
Bielefeld University[Affiliation] OR Universitaet Bielefeld[Affiliation] OR 
University of Bielefeld[Affiliation] OR Universität Bielefeld[Affiliation] AND (2017/01/01[PDat] : 2017/12/31[PDat]))" 


# 2018
myquery4 <- "(Universitat Bielefeld[Affiliation] OR University Bielefeld[Affiliation] OR 
Bielefeld University[Affiliation] OR Universitaet Bielefeld[Affiliation] OR 
University of Bielefeld[Affiliation] OR Universität Bielefeld[Affiliation] AND (2018/01/01[PDat] : 2018/12/31[PDat]))" 

# 2019
myquery5 <- "(Universitat Bielefeld[Affiliation] OR University Bielefeld[Affiliation] OR 
Bielefeld University[Affiliation] OR Universitaet Bielefeld[Affiliation] OR 
University of Bielefeld[Affiliation] OR Universität Bielefeld[Affiliation] AND (2019/01/01[PDat] : 2019/12/31[PDat]))" 
...
rm(pubmed_list1, pubmed_list2, pubmed_list3, pubmed_list4, pubmed_list5, 
   P1, P2, P3, P4, P5, X, myquery1, myquery2, myquery3, myquery4, myquery5, i, filename)
...
save.image("Workspaces/pubmedPrep_Bielefeld.Rdata")
...
quantile(results[["nAUperPaper"]], c(0.9, 0.91, 0.92, 0.93, 0.94, 0.95, 0.96, 0.97, 0.98,  0.99, 1))
# remove all papers which have more than 35 authors
which(results[["nAUperPaper"]] > 35)
data <- data[-which(results[["nAUperPaper"]] > 35),]

# keep only data from years 2015-2019
table(data$PY)
...
affilData <- read_excel("Data/databases/07_04_2021/wos_BI_1.xls")
for(i in 2:12){ 
  filename <- paste0("Data/databases/07_04_2021/wos_BI_", i, ".xls")
...
# remove spaces between doublenames to have more intersection between different spellings
doublenames <- which(str_detect(names(affil_wos), "[[:alpha:]]+[[:blank:]][[:alpha:]]+[[:blank:]]"))
names(affil_wos)[doublenames] <- unlist(lapply(str_split(names(affil_wos)[doublenames], " "), function(x) {
  paste(paste0(x[1:(length(x)-1)], collapse = ""), x[length(x)])
}))
...
# Scopus Affiliations
scopusData <- read_csv("Data/databases/07_04_2021/scopus_BI_2015.csv")
for(i in 2016:2019){ 
  filename <- paste0("Data/databases/07_04_2021/scopus_BI_", i, ".csv")
  X <- read_csv(filename)
  # merge data sets
...
names(affilScopus) <- str_replace_all(names(affilScopus), "[[:punct:]]", "")
# remove spaces between doublenames to have more intersection between different spellings
doublenames <- which(str_detect(names(affilScopus), "[[:alpha:]]+[[:blank:]][[:alpha:]]+[[:blank:]]"))
names(affilScopus)[doublenames] <- unlist(lapply(str_split(names(affilScopus)[doublenames], " "), function(x) {
  paste(paste0(x[1:(length(x)-1)], collapse = ""), x[length(x)])
}))
...
# Pubmed
singleString <- paste(readLines("Data/databases/07_04_2021/pubmed-Universita-set_edit.txt", encoding = "UTF-8"), collapse=" ")
...
names(affil_pubmed) <- str_replace(names(affil_pubmed), "AE", "A")
names(affil_pubmed) <- str_replace(names(affil_pubmed), "UE", "U")
names(affil_pubmed) <- str_replace(names(affil_pubmed), "OE", "O")

# remove spaces between doublenames to have more intersection between different spellings
doublenames <- which(str_detect(names(affilPubmed), "[[:alpha:]]+[[:blank:]][[:alpha:]]+[[:blank:]]"))
names(affilPubmed)[doublenames] <- unlist(lapply(str_split(names(affilPubmed)[doublenames], " "), function(x) {
  paste(paste0(x[1:(length(x)-1)], collapse = ""), x[length(x)])
}))
...
rm(cols, i, j, n, y, l, sep, filename)

save.image("Workspaces/workspaceBiData_DatPrep_Bibliometrix.Rdata")
